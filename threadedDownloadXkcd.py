...
def downloadXkcd(startComic, endComic):
    for urlNumber in range(startComic, endComic):
        ...
        comicElem = soup.select('#comic img') #comicmap
        if comicElem == []:
...
for i in range(0, 140, 10): #loops 14 times, creates 14 threads
    ...
    downloadThread = threading.Thread(target=downloadXkcd, args=(start, end)) #don't call downloadXkcd() yet
    downloadThreads.append(downloadThread)
    downloadThread.start() #now call downloadXkcd()
